	...
	def kill(self):
		...
		if self.is_alive:
			try:
				...
				# using .kill() on the Isolate sandbox directly sometimes results in a zombie process which requires superused privileges to kill
	#			time.sleep(3)
...
class IsolatedHouse(House):
	"""Provide an (in)secure sandbox which will run arbitrary commands
	through the Isolate module.

	The sandbox class is used to invoke arbitrary shell commands.
	This class provides the same interface as the secure Sandbox but doesn't
	require any special system setup. Instead of trying to create a secure
	environment directly, it invokes commands through Isolate. Comments on
	the security of this setup are welcome.
...
#		shell_command = [config.isolate_bin, "--box", str(self.box_id), "-d", self.origin, "-d", config.bin_dir, -d config.java_dir, "--env=HOME=" + self.working_directory, "--run", "--" ] + shell_command
	...
	def get_working_directory(self, box_id):
		return "/tmp/box/" + str(box_id) + "/box/"

	def get_isolated_box(self):
		result = -1
		test = 0
		while (result < 0):
			if (test > 99): test = 0
